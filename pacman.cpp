...
void carregarMapa(){
     ...
     for (int i = 0; i < 26; i++)
        for (int j = 0; j < 26; j++) {
            if (MAPA[i][j]=='1') al_draw_bitmap(wall, j*q, i*q, 0);
            else if (MAPA[i][j]=='0') { 
                if (posx == j*q && posy == i*q) MAPA[i][j]='a';
                else al_draw_bitmap(pill, j*q, i*q, 0);
            }
            if ((i==1 && j==1) || (i==1 && j==23) || (i==23 && j==1) || (i==23 && j==23)) al_draw_bitmap(ghost, j*q, i*q, 0);
        }
